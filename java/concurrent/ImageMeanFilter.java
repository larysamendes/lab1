...
public class ImageMeanFilter {
     ...
     */
    public static void applyMeanFilter(String inputPath, String outputPath, int kernelSize, int total_threads) throws IOException {
        // Load image
        ...
        int tamanho = height / total_threads;

        List<Thread> threads = new LinkedList<>();

        for (int th = 0; th < height; th +=tamanho){
            int inicio = th;
            int fim = inicio + tamanho - 1;
            Thread thread = new Thread(() -> processPixel(inicio, fim,  width, originalImage, kernelSize, filteredImage));
            threads.add(thread);
            thread.start();
        }
    ...
    static public void processPixel(int inicio, int fim, int width, BufferedImage originalImage, int kernelSize, BufferedImage filteredImage){
    ...
    public static void main(String[] args) {
        ...
        try {
            applyMeanFilter(inputFile, "filtered_output.jpg", 7, Integer.parseInt(args[1]));
        } catch (IOException e) {
